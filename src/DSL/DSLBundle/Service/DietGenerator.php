...
class DietGenerator
{
    ...
    public function generate(DietRules $rule) {
        ...
        if ($rule->getDailyCaloriesRequirementsKcal() ||
            $rule->getDailyProteinRequirementsG() ||
            $rule->getDailyCarbohydratesRequirementsG() ||
            $rule->getDailyFatRequirementsG()
        ) {
            $calculationType = new CompositionType();
        } elseif ($rule->getMonthlyCost()) {
